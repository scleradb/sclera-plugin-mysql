...
package com.scleradb.plugin.dbms.rdbms.mysql.service
...
class MySQLService extends DBService {
    override val id: String = MySQL.id
    ...
    override def createLocation(
        ...
        dbSchemaOpt.foreach { s =>
            if( s.toUpperCase != paramDbName )
                throw new IllegalArgumentException(
                    "MySQL does not support a schema \"" + s +
                    "\" different from the database \"" + paramDbName + "\""
                )
        }

        new MySQL(schema, locationId, paramDbName, config, permit)
    }
}
